...
description: 创建 React 组件、页面及样式的开发规范。
---
...
1. **视图层**：`src/pages/` 与 `src/components/`，仅负责展示和交互。
2. **状态层**：`src/stores/`，负责逻辑与状态。
3. **API 层**：`src/apis/`，定义端点与请求。

## 开发规范

- **中文注释**：代码必须尽可能详细书写中文注释；核心逻辑与易踩坑处必须配简洁明了的中文解释。
- **命名**：组件命名使用 PascalCase。
- **TypeScript**：避免使用 `any`，在 `src/types` 定义类型。
...
- **Modal**：编辑类的Modal要设置maskClosable={false}，单纯展示类可不设置。内容部分尽量限制最大高度，控制在Modal内部进行滚动。
