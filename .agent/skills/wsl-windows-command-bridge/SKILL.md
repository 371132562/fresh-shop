...
description: 在 WSL 中操作位于 Windows 文件系统的 fresh-shop 时使用，统一决定何时改走 Windows PowerShell 执行依赖相关命令。
---
...
1. 当前运行环境是 WSL
2. 项目目录位于 Windows 文件系统，例如 `/mnt/d/work/fresh-shop`
...
- Prisma、SQLite、`swc`、`rollup` 等依赖因宿主环境不一致报错
- 同一 SQLite 文件被 WSL 与 Windows 进程混用，出现 `disk I/O error`、WAL/SHM 异常或锁文件问题
...
```bash
powershell.exe -NoProfile -Command "Set-Location 'D:\work\fresh-shop\backend'; pnpm exec prisma generate"
```

前端示例：

```bash
powershell.exe -NoProfile -Command "Set-Location 'D:\work\fresh-shop\frontend'; pnpm lint"
```
...
3. Windows 路径使用盘符路径，例如 `D:\work\fresh-shop\backend`。
