...
│   │   ├── router.tsx         # 路由配置
│   │   └── main.tsx           # 应用入口文件
│   └── .env                   # 前端环境变量（如 VITE_API_BASE_URL=/fresh）
├── backend/                   # 后端（NestJS）
│   ├── src/
│   │   ├── businessComponent/ # 业务模块（controller/service/module）
│   │   ├── interceptors/      # 响应拦截器（统一响应结构）
│   │   ├── exceptions/        # 全局异常过滤器
│   │   └── main.ts            # 入口（全局前缀 /fresh）
...
- **异步 action**：统一 `try/catch/finally` 管理 loading，并在成功后刷新列表/统计（参考 `frontend/src/stores/orderStore.ts`）。
- **请求封装**：基础请求封装于 `frontend/src/services/base.ts`（基于 fetch），端点常量集中在 `frontend/src/services/apis.ts`；默认 `baseURL` 读取 `VITE_API_BASE_URL`（开发环境通常为 `/fresh`，与后端全局前缀保持一致）。
...
本项目包含较多统计与分析页面，口径需要前后端保持一致：

- 统计分析的全局口径以 `backend/README.statistics.md` 为准（时间过滤、订单集合、销售额/退款/利润/订单量等定义）。
- 前端如需对口径进行解释性展示，优先复用/扩展 `frontend/src/components/StatisticsGuideModal`，避免每个页面重复编写。
